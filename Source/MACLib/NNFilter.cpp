...
void CNNFilter::AdaptSSE(int * pM, int * pAdapt, int64 nDirection, intn nOrder)
{
    ...
    if (nDirection < 0)
    {
        ...
        {
            __m128i sseM = _mm_loadu_si128((__m128i *) &pM[z]);
...
#ifdef ENABLE_AVX_ASSEMBLY
void CNNFilter::AdaptAVX(int * pM, int * pAdapt, int64 nDirection, intn nOrder)
{
    // we require that pM is aligned, allowing faster loads and stores
    ASSERT((size_t(pM) % 16) == 0);
    ...
    if (nDirection < 0)
    {
        for (int z = 0; z < nOrder; z += 8)
        {
            __m256i sseM = _mm256_loadu_si256((__m256i *) &pM[z]);
            __m256i sseAdapt = _mm256_loadu_si256((__m256i *) &pAdapt[z]);
            __m256i sseNew = _mm256_add_epi32(sseM, sseAdapt);
            _mm256_store_si256((__m256i *) & pM[z], sseNew);
        }
    }
    else if (nDirection > 0)
    {
        for (int z = 0; z < nOrder; z += 8)
        {
            __m256i sseM = _mm256_loadu_si256((__m256i *) &pM[z]);
            __m256i sseAdapt = _mm256_loadu_si256((__m256i *) &pAdapt[z]);
            __m256i sseNew = _mm256_sub_epi32(sseM, sseAdapt);
            _mm256_store_si256((__m256i *) &pM[z], sseNew);
        }
    }
...
void CNNFilter::AdaptAVXx16(short * pM, short * pAdapt, int64 nDirection, intn nOrder)
{
    // we require that pM is aligned, allowing faster loads and stores
    ...
    if (nDirection < 0)
    {
        ...
        {
            __m256i sseM = _mm256_load_si256((__m256i *) &pM[z]);
            __m256i sseAdapt = _mm256_loadu_si256((__m256i *) &pAdapt[z]);
            __m256i sseNew = _mm256_add_epi16(sseM, sseAdapt);
            _mm256_store_si256((__m256i *) &pM[z], sseNew);
        }
    }
    else if (nDirection > 0)
    {
        for (int z = 0; z < nOrder; z += 16)
        {
            __m256i sseM = _mm256_load_si256((__m256i *) &pM[z]);
            __m256i sseAdapt = _mm256_loadu_si256((__m256i *) &pAdapt[z]);
            __m256i sseNew = _mm256_sub_epi16(sseM, sseAdapt);
            _mm256_store_si256((__m256i *) &pM[z], sseNew);
        }
    }
...
int64 CNNFilter::CalculateDotProductAVXx16(short * pA, short * pB, intn nOrder)
{
    // we require that pB is aligned, allowing faster loads
    ...
    // loop
    __m256i sseSum = _mm256_setzero_si256();
    for (int z = 0; z < nOrder; z += 16)
    {
        __m256i sseA = _mm256_loadu_si256((__m256i *) &pA[z]);
        __m256i sseB = _mm256_load_si256((__m256i *) &pB[z]);
        __m256i sseDotProduct = _mm256_madd_epi16(sseA, sseB);
        sseSum = _mm256_add_epi32(sseSum, sseDotProduct);
    }

    // build output
    int64 nDotProduct = sseSum.m256i_i32[0] + sseSum.m256i_i32[1] + sseSum.m256i_i32[2] + sseSum.m256i_i32[3] +
        sseSum.m256i_i32[4] + sseSum.m256i_i32[5] + sseSum.m256i_i32[6] + sseSum.m256i_i32[7];
